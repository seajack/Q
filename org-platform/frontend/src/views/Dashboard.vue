    ...
    <div class="chart-grid">
      <div class="chart-card large">
        ...
        <div class="chart-body">
          <div v-if="departmentLevels.length" ref="deptLevelChart" class="chart-container"></div>
      ...
      <div class="chart-card">
        ...
        <div class="chart-body">
          <div v-if="positionLevels.length" ref="positionLevelChart" class="chart-container"></div>
      ...
      <div class="chart-card">
        <div class="chart-header">
          <h3>员工能力雷达图</h3>
          <span class="chart-sub">多维度比较</span>
        </div>
        <div class="chart-body">
          <div v-if="employeeSkills.length" ref="radarChart" class="chart-container"></div>
          <div v-else class="empty-placeholder">暂无员工能力数据</div>
        </div>
      </div>
    </div>
...
// 图表实例
const deptLevelChart = ref<HTMLElement | null>(null)
const positionLevelChart = ref<HTMLElement | null>(null)
const radarChart = ref<HTMLElement | null>(null)
let deptLevelChartInstance: echarts.ECharts | null = null
let positionLevelChartInstance: echarts.ECharts | null = null
...
// 更新部门层级图表
const updateDeptLevelChart = () => {
  if (!deptLevelChartInstance) return
  ...
  deptLevelChartInstance.setOption(option)
}

// 更新职位级别图表
const updatePositionLevelChart = () => {
  if (!positionLevelChartInstance) return
  ...
  positionLevelChartInstance.setOption(option)
}

// 更新雷达图
const updateRadarChart = () => {
  if (!radarChartInstance || !employeeSkills.value.length) return
...
// 监听数据变化更新图表
watch([departmentLevels, positionLevels, employeeSkills], () => {
  nextTick(() => {
    updateDeptLevelChart()
    updatePositionLevelChart()
    updateRadarChart()
...
// 监听窗口大小变化
const handleResize = () => {
  deptLevelChartInstance?.resize()
  positionLevelChartInstance?.resize()
...
onBeforeUnmount(() => {
  ...
  deptLevelChartInstance?.dispose()
  positionLevelChartInstance?.dispose()
