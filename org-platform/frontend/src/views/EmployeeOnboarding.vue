...
// 计算属性
const filteredOnboardingList = computed(() => {
  let filtered = onboardingList.value
...
const handleView = (row) => {
  ElMessage.info(`查看 ${row.employeeName} 的入职信息`)
}

const handleComplete = (row) => {
  ElMessageBox.confirm('确定要完成该员工的入职流程吗？', '提示', {
    ...
    row.progress = 100
    ElMessage.success('入职流程已完成')
  })
}

const handleCancel = (row) => {
  ElMessageBox.confirm('确定要取消该员工的入职流程吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
  ...
  formRef.value.validate((valid) => {
    if (valid) {
      if (isEdit.value) {
        ...
        onboardingList.value.unshift(newItem)
        ElMessage.success('入职信息添加成功')
      }
